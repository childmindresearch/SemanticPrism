...
async def run_diagnostics():
    global extractor, workflow_errors, master_context, start_datetime, start_time, all_themes, raw_triples, original_subjs, original_preds, original_objs, norm_subjs, norm_preds, norm_objs, hypernyms, synthesizer, proposed_clusters, verified_clusters, hypernym_mapping, mapper, mapped_triples, topology, graph, partition, hierarchy, resolved_schemas, file_path
    ...
    # --- HYPERGRAPH ADDITION ---
    hyper_data = topology.build_hypergraph_topology(mapped_triples)
    hyper_html = topology.visualize_hypergraph(hyper_data["B"], "outputs/05_topology")
    IPython.display.display(IPython.display.IFrame(hyper_html, width="100%", height="600px"))
