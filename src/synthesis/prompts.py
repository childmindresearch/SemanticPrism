...
You will receive a subset of Semantic Triples representing a community cluster.
Your goal is to define Pydantic (v2) `BaseModel` classes that model the relationships and processes in this cluster for downstream LLM extraction.
...
2. **Inheritance Logic:** If the dominant theme of these triples matches a `child_theme` in `ThemeInheritance`, subclass its corresponding parent schema. Otherwise, generate an independent root schema.
...
4. **Default to Optional:** Wrap all generated fields in `Optional[...] = None` to prevent downstream extraction models from hallucinating missing data.
5. **No Unique IDs:** Do not include arbitrary identifier fields like `id` or `uuid`.
6. **Self-Contained Code:** Output pure Python code containing all necessary imports. Do not wrap in markdown code blocks."""

NODE2VEC_SCHEMA_SYNTHESIS_PROMPT = """You are an Ontological Python Architect.
You will receive a subset of Semantic Triples representing a structural category.
Your goal is to define Pydantic (v2) `BaseModel` classes that model this ontological category for downstream LLM extraction.
